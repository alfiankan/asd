...
class Arreha(object):
    ...
    def simpan(self,data):
        """method untuk memparsing data baris kolom berupas string menjadi sebuah object 
        lalu dimasukan kedalamm array yang kita buat"""
        ...
        for x in range(len(self._internal)):
            ...
            new = Perang((listobject[x][0],listobject[x][1],listobject[x][2],listobject[x][3],listobject[x][4]))
            self.__setitem__(x,new)
    def lihat(self,index):
        """method untuk menampilkan nama barang berdasarkan inddex"""
        return self._internal[index].nama
...
# panggil method lihat data index ke 14
print(arrayATK.lihat(14))